class OptionsGUI {
    ...
    */
    ...
    /**
    * Add a label to the screen (if you imported MathJax, uses LaTeX to display)
    * @param name   Name of the label
    * @param value  Value of the label (default '')
    * @param folder Reference of the folder (default this.datas.configuration)
    */
    addLabel(name, value = '', folder = this.datas.configuration) {
        let params = {};
        params[name] = value;
        let newLabel = folder._folder.addInput(params, name);

        if (this.usesMathJax)
            this.processHTMLLaTeX(newLabel.controller.view.elem_.children[0], 'element');
    }
    ...
    * @return A function that returns the current value of the input
    */
    addList(name, values, defaultValue, folder = this.datas.configuration, callback) {
        folder._params[name] = defaultValue;
        folder._values[name] = defaultValue;
        let newInput = folder._folder
